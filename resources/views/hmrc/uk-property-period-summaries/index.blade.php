    ...
    <div class="container-fluid mt-4">
        ...
        <div class="hmrc-page-header">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h4 class="page-title mb-1">UK Property Period Summaries</h4>
                    <p class="text-muted mb-0" style="font-size: 0.875rem;">Manage quarterly and period submissions for your UK property businesses</p>
                </div>
            </div>
            <div>
                <a href="{{ route('hmrc.uk-property-period-summaries.create') }}" class="btn btn-hmrc-primary">
                    <i class="fas fa-plus me-2"></i> New Period Summary
                </a>
            </div>
        </div>
            ...
            <div class="stat-card stat-card-info">
                <div class="stat-card-body">
                    <div class="stat-card-content">
                        <p class="stat-card-label">Total Summaries</p>
        ...
        <div class="hmrc-card mb-4">
            <form method="GET" action="{{ route('hmrc.uk-property-period-summaries.index') }}" class="row g-3">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <label for="business_id" class="form-label text-muted" style="font-size: 0.875rem; font-weight: 500;">Business</label>
        ...
        <x-hmrc.obligations-section
            ...
            title="Periodic Obligations (UK Property)"
        />
        ...
        <div class="hmrc-card">
            @if($summaries->isEmpty())
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted">No UK Property Period Summaries Found</h5>
                    <p class="text-muted mb-4">Create your first UK property period summary to get started.</p>
                    <a href="{{ route('hmrc.uk-property-period-summaries.create') }}" class="btn btn-hmrc-primary">
                        <i class="fas fa-plus me-2"></i> Create Period Summary
                    </a>
                </div>
            @else
                    ...
                    <table class="hmrc-table">
                        ...
                        <tbody>
                            @foreach($summaries as $summary)
                                <tr>
                                    <td>
                                        <strong>{{ $summary->tax_year }}</strong>
                                    </td>
                                    <td>
                                        {{ $summary->from_date->format('d M Y') }} - {{ $summary->to_date->format('d M Y') }}
                                        <br>
                                    ...
                                    <td>
                                        ...
                                        <br>
                                        <small class="text-muted">{{ $summary->business?->type_of_business ?? '' }}</small>
                                    </td>
                                    <td>
                                        <small>
                                            @if($summary->isUnifiedProperty())
                                                {{-- Tax Year 2025-26+: Unified Property --}}
                                                @if($summary->total_uk_property_income > 0)
                                                    UK Property: <span class="text-success fw-bold">£{{ number_format($summary->total_uk_property_income, 2) }}</span>
                                                @else
                                                    <span class="text-muted">-</span>
                                                @endif
                                            @else
                                                {{-- Tax Year < 2025-26: Separate FHL and Non-FHL --}}
                                                @if($summary->total_fhl_income > 0)
                                                    FHL: <span class="text-success fw-bold">£{{ number_format($summary->total_fhl_income, 2) }}</span><br>
                                                @endif
                                                @if($summary->total_non_fhl_income > 0)
                                                    Non-FHL: <span class="text-success fw-bold">£{{ number_format($summary->total_non_fhl_income, 2) }}</span>
                                                @endif
                                    ...
                                    <td>
                                        <small>
                                            @if($summary->isUnifiedProperty())
                                                {{-- Tax Year 2025-26+: Unified Property --}}
                                                @if($summary->total_uk_property_expenses > 0)
                                                    UK Property: <span class="text-danger fw-bold">£{{ number_format($summary->total_uk_property_expenses, 2) }}</span>
                                                @else
                                                    <span class="text-muted">-</span>
                                                @endif
                                            @else
                                                {{-- Tax Year < 2025-26: Separate FHL and Non-FHL --}}
                                                @if($summary->total_fhl_expenses > 0)
                                                    FHL: <span class="text-danger fw-bold">£{{ number_format($summary->total_fhl_expenses, 2) }}</span><br>
                                                @endif
                                                @if($summary->total_non_fhl_expenses > 0)
                                                    Non-FHL: <span class="text-danger fw-bold">£{{ number_format($summary->total_non_fhl_expenses, 2) }}</span>
                                                @endif
                                    ...
                                    <td>
                                        @if($summary->submission_date)
                                            {{ $summary->submission_date->format('d M Y') }}
                                            <br>
                                            <small class="text-muted">{{ $summary->submission_date->format('H:i') }}</small>
                                        @else
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="{{ route('hmrc.uk-property-period-summaries.show', $summary) }}"
