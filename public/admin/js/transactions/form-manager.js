 ...
 * - ✅ FIXED: Loads suppliers for purchase types
 */
...
class FormManager {
    ...
    /**
     * ✅ FIXED: Load form data with proper supplier support
     */
    loadFormData(currentPaymentType) {
        const purchaseTypes = ['purchase', 'purchase_credit'];
        const needsSuppliers = purchaseTypes.includes(currentPaymentType);

        // ✅ Load the correct data first
        const loadPromises = [
            window.dataLoader.loadLedgerRefsForDropdown()
        ];

        // ✅ Load suppliers OR customers based on payment type
        if (needsSuppliers) {
            console.log('📦 Loading SUPPLIERS for payment type:', currentPaymentType);
            loadPromises.push(window.dataLoader.loadSuppliersForDropdown());
        } else {
            console.log('👥 Loading CUSTOMERS for payment type:', currentPaymentType);
            loadPromises.push(window.dataLoader.loadCustomersForDropdown());
        }
        ...
        Promise.all(loadPromises).then(() => {
            ...
            if (needsSuppliers) {
                console.log('📦 Populating suppliers dropdown');
                if (window.dataLoader.populateSuppliersDropdown) {
                    window.dataLoader.populateSuppliersDropdown();
                } else {
                    console.error('❌ populateSuppliersDropdown method not found');
                }
            } else {
    ...
    /**
     * ✅ FIXED: Fallback data loading with supplier support
     */
    loadFormDataFallback(currentPaymentType) {
        const purchaseTypes = ['purchase', 'purchase_credit'];

        if (purchaseTypes.includes(currentPaymentType)) {
            console.log('📦 Loading SUPPLIERS (fallback) for:', currentPaymentType);
            if (window.dataLoader.populateSuppliersDropdown) {
                window.dataLoader.populateSuppliersDropdown();
            }
        } else {
            console.log('👥 Loading CUSTOMERS (fallback) for:', currentPaymentType);
            window.dataLoader.populateCustomerDropdown();
        }
