...
class TransactionCodeManager {
    constructor() {
        ...
        // ✅ CONTEXT-AWARE: Detect if we're in company module or main app
        this.isCompanyModule = window.location.pathname.startsWith('/company');

        // ✅ Set correct URLs based on context
        this.GENERATE_URL = this.isCompanyModule
            ? window.location.origin + "/company/invoices/generate-auto-code"
            : window.location.origin + "/transactions/generate-auto-code";

        this.CHECK_URL = this.isCompanyModule
            ? window.location.origin + "/company/invoices/check-code-unique"
            : window.location.origin + "/transactions/check-code-unique";

        this.codeMinSuffix = 1;
        this.codeManual = false;

        console.log('🔍 Code Manager Context:', {
            isCompanyModule: this.isCompanyModule,
            generateUrl: this.GENERATE_URL,
    ...
    async refreshAutoCodeFor(type) {
        ...
        console.log('🔄 Refreshing auto code:', {
            type: type,
        ...
        // ✅ Add company_id for company module
        ...
        const res = await fetch(this.GENERATE_URL, {  // ✅ Use context-aware URL
            method: 'POST',
            headers: {
        ...
        console.log('✅ Auto code received:', data.auto_code);
        this.applyAutoCodeToInvoice(data.auto_code);
    }

    /**
 * ✅ NEW: Get current company ID (for company module)
 */
    getCurrentCompanyId() {
        ...
        const hiddenField = document.getElementById('current_company_id');
        if (hiddenField) {
