    ...
    loadVatTypesForProductEdit(prefix, product) {
        ...
        if (window.vatManager && typeof window.vatManager.loadVatTypesByForm === 'function') {
            window.vatManager.loadVatTypesByForm(paymentType, (vatTypes) => {
                ...
                // Set the VAT value after dropdown is populated
                if (product.vat_rate_id) {
                    // Try to find by vat_rate_id (database ID)
                    const vatOption = Array.from(vatSelect.options).find(opt =>
                        opt.dataset.vatId == product.vat_rate_id
                    );

                    if (vatOption) {
                        vatSelect.value = vatOption.value; // This is the percentage
                        console.log('✅ VAT rate set to:', vatOption.value + '%');
                    } else {
                        console.warn('⚠️ VAT option not found for ID:', product.vat_rate_id);
                    }
                }
            });
        } else {
 ...
 */
        ...
        fetch(url, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Check if we're on the transaction form or index page
                    if (!this.isIndexPage) {
                        // Auto-fill invoice row with new product (only on transaction form)
    ...
    selectProduct(productId, rowIndex) {
        ...
        if (vatSelect && product.vat_rate_id) {
            const vatOption = Array.from(vatSelect.options).find(opt =>
                opt.dataset.vatId == product.vat_rate_id
            );
            if (vatOption) {
                ...
                if (vatIdField) {
                    vatIdField.value = product.vat_rate_id;
                }
                console.log('✅ Auto-filled VAT:', vatOption.value + '%');
            }
        }
